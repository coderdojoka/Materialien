...
Hier gibt es zwei neue Variablen, \code{zahl} und \code{ergebnis}. Die Variable \code{ergebnis} bekommt als Wert das Ergebnis der Berechnung von \code{zahl * 5} zugewiesen.
...
Hier gilt einfach die Merkregel von oben. Zuerst wird die rechte Seite berechnet. Es wird der Wert von \code{zahl} gelesen und \code{1} dazu addiert. Dann wird auf der linken Seite, der Wert von \code{zahl} auf den neu ausgerechneten Wert gesetzt.
...
\begin{itemize}
	...
	\item Gib den Wert deiner Variablen mit \code{print} aus und sieh wie er sich verändert
	\item Überschreibe den Wert deiner Variablen mit 10-fachen des ursprünglichen Wertes
	\item Finde heraus, welche Buchstaben, Zahlen, Sonderzeichen in Variablen erlaubt sind. Um dies auszuprobieren starte mit einem Namen der nur Buchstaben enthält, z.B. \code{zahl}. Starte das Programm, es sollte erfolgreich durchlaufen. Füge nun neue Zeichen hinzu und teste erneut das Program. 
\end{itemize}
