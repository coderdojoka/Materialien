...
Die funktioniert mithilfe von \code{input()}.

\inputminted[firstline=1, lastline=2]{python}{../../../Beispiele/eingabe.py}
Was in den Klammern bei \code{input(...)} steht wird zuerst auf der Konsole ausgeben, dann wir auf einen Eingabe des Benutzers gewartet.\\
...
Mittles \code{input()} kann nur Text eingelsen werden! Auch wenn der Benutzer \code{12} eintippt, so wird dies als Text \code{"12"} interpretiert.
...
Hier wird nun zuerst das Alter als Text eingelsen und dann konvertiert. Die Variable \code{alter} enthält zunächst also einen Text und danach die Zahl, die dieser Text darstellt.
